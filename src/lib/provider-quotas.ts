...
import type { ProviderQuota, QuotaProviderId, QuotaResponse, QuotaWindow } from "./quota-types";
...
export function parseQuotas(id: QuotaProviderId, raw: unknown): QuotaWindow[] {
  ...
  return Object.entries(record(data.models)).flatMap(([key, value]) => {
...
const requestQuota: RequestQuota = async (id, auth) => {
  ...
  if (codex && typeof auth.accountId === "string") headers["ChatGPT-Account-Id"] = auth.accountId;
  if (!codex) Object.assign(headers, { "Content-Type": "application/json", "User-Agent": "antigravity/1.107.0", "X-Client-Name": "antigravity" });
  const response = await proxyFetch(codex ? "https://chatgpt.com/backend-api/wham/usage"
    : "https://cloudcode-pa.googleapis.com/v1internal:fetchAvailableModels", {
    dispatcher, method: codex ? "GET" : "POST", headers,
    ...(codex ? {} : { body: JSON.stringify(typeof auth.projectId === "string" ? { project: auth.projectId } : {}) }),
      ...
      current.pending = (async () => {
        ...
        try {
          ...
          if (response.status === 401) result = failure("expired", "登录已失效，请在 Pi 中重新登录后重试");
            ...
            const plan = record(response.data).plan_type;
